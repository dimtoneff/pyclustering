...
class fsync_dynamic:
    ...
    def __getitem__(self, index):
        ...
        if index == 0:
            ...
            raise NameError('Out of range ' + index +
...
class fsync_visualizer:
    ...
    def show_output_dynamic(fsync_output_dynamic):
        ...
        figure, _ = pyclustering.utils.draw_dynamics(
            fsync_output_dynamic.time, fsync_output_dynamic.output, x_title="t", y_title="amplitude")
    ...
    def show_output_dynamics(fsync_output_dynamics):
        ...
        pyclustering.utils.draw_dynamics_set(
            fsync_output_dynamics, "t", "amplitude", None, None, False, False)
...
class fsync_network(network):
    ...
    def __init__(self, num_osc, factor_frequency=1.0, factor_radius=1.0, factor_coupling=1.0, type_conn=conn_type.ALL_TO_ALL, representation=conn_represent.MATRIX):
        ...
        self.__frequency = factor_frequency if isinstance(factor_frequency, list) else [
            fsync_network.__DEFAULT_FREQUENCY_VALUE * factor_frequency for _ in range(num_osc)]
        self.__radius = factor_radius if isinstance(factor_radius, list) else [
            fsync_network.__DEFAULT_RADIUS_VALUE * factor_radius for _ in range(num_osc)]
        self.__coupling_strength = fsync_network.__DEFAULT_COUPLING_STRENGTH * factor_coupling
        self.__properties = [self.__oscillator_property(
            index) for index in range(self._num_osc)]
    ...
    def simulate(self, steps, time, collect_dynamic=False):
        ...
        dynamic_amplitude, dynamic_time = (
            [], []) if collect_dynamic is False else ([self.__amplitude], [0])
        ...
        for t in numpy.arange(step, time + step, step):
            ...
            if collect_dynamic is True:
                dynamic_amplitude.append(
                    [numpy.real(amplitude)[0] for amplitude in self.__amplitude])
                dynamic_time.append(t)

        if collect_dynamic is False:
            dynamic_amplitude.append([numpy.real(amplitude)[0]
                                     for amplitude in self.__amplitude])
    ...
    def __calculate(self, t, step, int_step):
        ...
        for index in range(0, self._num_osc, 1):
            z = numpy.array(
                self.__amplitude[index], dtype=numpy.complex128, ndmin=1)
            result = odeint(self.__calculate_amplitude, z.view(
                numpy.float64), numpy.arange(t - step, t, int_step), (index, ))
            next_amplitudes[index] = (
                result[len(result) - 1]).view(numpy.complex128)
    ...
    def __synchronization_mechanism(self, amplitude, index):
        ...
        for k in range(self._num_osc):
            if self.has_connection(index, k) is True:
                amplitude_neighbor = numpy.array(
                    self.__amplitude[k], dtype=numpy.complex128, ndmin=1)
    ...
    def __calculate_amplitude(self, amplitude, t, argv):
        ...
        dzdt = self.__landau_stuart(
            z, argv) + self.__synchronization_mechanism(z, argv)
